...
class Uptime(commands.Cog):
    ...
    def get_timestamps(self):
        ...
        return {"relative": f"<t:{ts}:R>", "full": f"<t:{ts}:F>"}
    ...
    async def uptime_slash(self, interaction: discord.Interaction):
        ...
        embed.add_field(name="Running for", value=f"```{uptime_str}```", inline=True)
        embed.add_field(name="Since", value=timestamps["relative"], inline=True)
        embed.add_field(name="Started", value=timestamps["full"], inline=True)
